...
<div class="relative min-h-svh px-4 py-10">
	...
	<div class="mx-auto w-full max-w-2xl space-y-6">
		<div>
			...
			<p class="text-sm text-muted-foreground">
				Each project runs its own isolated auth agent, backed by its own database.
			</p>
		</div>

		{#if data.projects.length}
			<div class="grid gap-3" data-testid="project-list">
				{#each data.projects as project (project.id)}
					<a href={resolve('/dashboard/[projectId]', { projectId: project.id })} class="block">
						<Card.Root class="transition-colors hover:border-primary">
							<Card.Header>
								<Card.Title>{project.name}</Card.Title>
								<Card.Description>{project.id}</Card.Description>
							</Card.Header>
						</Card.Root>
					</a>
				{/each}
