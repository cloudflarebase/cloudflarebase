...
- `drizzle/`: drizzle-kit output. `src/migrations.ts` inlines that SQL as string literals and is what `onStart` applies — generated by `scripts/generate-migrations.mjs`, never hand-edited. Inlining means the agent needs no Wrangler Text-module rule, so it can be consumed as a plain npm dependency.
- `src/env.d.ts`: optional secrets and variables not represented by generated Wrangler types.

## Schema changes

1. Edit `src/db/schema.ts`.
2. Run `npm run migrations` (drizzle-kit generate, then inline the SQL into `src/migrations.ts`).
...
- **`console`** is the dashboard's own operator auth. It refuses guest sign-in outright, and accepts exactly one `sign-up/email` — the first-run owner claim — rejecting the rest with 403. Under `DEMO_MODE` it refuses the claim entirely: a demo deployment has no operators, so leaving a first-come claim open on a public URL would only let a stranger take a console nobody is meant to use. Mirrored as `CONSOLE_PROJECT_ID` in the app's `src/lib/console.ts`. The route check is not the whole story: social sign-in creates users implicitly on the OAuth callback without touching any sign-up route, so the invariant — at most one console user, none under `DEMO_MODE` — is also enforced where every path converges, a `denyUserCreation` veto in the user-creation database hook (`auth.ts`). Without it, configuring Google credentials would quietly reopen console registration to anyone with a Google account. Social **sign-in** for the existing owner still works; an unknown account bounces with 403 instead of registering.
