...
- The project agent is a docked, resizable right-hand pane in the dashboard layout (PaneForge through the shadcn `resizable` wrapper): open by default on desktop, collapsible to a slim rail, open state in `localStorage`, pane widths persisted via PaneForge's `autoSaveId`. Below 768px (shared `IsMobile` hook) a bottom Backend | Agent tab bar swaps the main content for a full-screen agent view. The dashboard shell is viewport-height; pages scroll inside `<main>`, not the window.
