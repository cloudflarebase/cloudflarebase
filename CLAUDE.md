...
- `AuthAgent` owns CORS: environment `TRUSTED_ORIGINS` plus per-project allowed origins, exact-origin echo, credentials, exposes `set-auth-token`. Never enable `routeAgentRequest(..., { cors: true })`; its wildcard overrides the project policy.
...
- `TRUSTED_ORIGINS` is empty in the self-hosted default and must be set after first deploy. It is the CSRF allowlist; failure surfaces as a rejected credential, not a config error.
